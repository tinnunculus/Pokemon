...
class TrainState(flax.struct.PyTreeNode):
    ...
    def apply_gradients(self, *, grads, **kwargs):
        ...
        """
        if self.tx is None:
            return self.replace(step=self.step + 1, **kwargs)

        # `jax.grad` can return a plain nested dict for the gradient tree even when
        # `self.params` is a Flax `FrozenDict`. `optax` expects the same pytree node
        # type on both sides, so normalize the gradient container here.
    ...
    def apply_loss_fn(self, *, loss_fn, pmap_axis=None, has_aux=False):
        ...
        if has_aux:
            ...
            if isinstance(self.params, flax.core.FrozenDict) and not isinstance(grads, flax.core.FrozenDict):
                grads = flax.core.freeze(grads)
